...
/**
 * Returns an array of n elements randomly picked from arr.
 * The returned array does not contain duplicate elements.
 * The original array is modified to remove the picked elements.
 */
function pick<T>(arr: T[], n: number) {
  // Create a shallow copy of the original array to avoid mutating it (Side Effects)
  const a = [...arr];

  // Initialize an empty array to store the selected items
  const out: T[] = [];

  // Loop until we have 'n' items or we run out of items in the source copy
  for (let i = 0; i < n && a.length; i++) {
    // Generate a random integer index based on the current length of the copy
    const idx = Math.floor(Math.random() * a.length);

    // Remove 1 element at the random index using 'splice'.
    // Since 'splice' returns an array of deleted items, we take the first element [0].
    // Add the extracted item to our output collection
    out.push(a.splice(idx, 1)[0]);
  }
  return out; // Return the final collection of unique picked items
}

/**
 * Generates a random integer between a specified range, including both boundaries.
 * @param min - The lower bound of the range (inclusive).
 * @param max - The upper bound of the range (inclusive).
 * @returns A random integer between min and max.
 */
function randInt(min: number, max: number) {
  // 1. Calculate the total number of possible integers in the range (the "width").
  // We add +1 to make the 'max' value inclusive in the selection.
  const rangeWidth = max - min + 1;

  // 2. Generate a random decimal between 0 and 1, multiply it by the width,
  // and round it down to get a random step starting from zero.
  const randomStep = Math.floor(Math.random() * rangeWidth);

  // 3. Add the 'min' value to shift the result from "starting at zero"
  // to "starting at our minimum bound" (the offset).
...
async function seed() {
  try {
    ...
    // 1. Define the initial data for the 'genders' table.
    // We use .parse() to ensure each object strictly follows our Zod schema rules
    // (e.g., checking for required fields and correct data types) before proceeding.
    const genderRows = [
    ...
    // 2. Iterate through each validated row to synchronize with the database.
    for (const row of genderRows) {
      // 3. Perform a lookup to check if a gender with the same slug already exists.
      // We limit the result to 1 since slugs are unique identifiers.
      ...
      // 4. Conditional Insertion: only add the row if no matching record was found.
      // This makes the script "idempotent," meaning it can run multiple times
      // without creating duplicate entries or crashing.
      if (!exists.length) await db.insert(genders).values(row);
    }

    // 1. Define the raw color data and transform it using .map().
    // We apply the Zod validation schema to every element in the array at once.
    // The 'as RGBHex' is a Type Assertion to satisfy specific string format requirements.
    const colorRows = [
    ...
    // 2. Process each color sequentially to avoid duplicates in the database.
    for (const row of colorRows) {
      // 3. Check if the color already exists based on its unique slug.
      const exists = await db
        .select()
        .from(colors)
      ...
      // 4. Insert the color record only if it's missing (Idempotency check).
